...
import { getUserRepostedPosts } from "@/actions/profile.action";
import { getProfileByUsername } from "@/actions/profile.action";
import DeleteCited from "./DeleteCited";
import PostCard from "./PostCard";

type RepostedPosts = Awaited<ReturnType<typeof getUserRepostedPosts>>;
type Repost = RepostedPosts[number];
...
function UserReposts({
  ...
  const repostText =
    dbUserId === ownUser.id
      ? "Has compartido esto"
      : `${ownUser.name} ha compartido esto`;
  ...
  return (
    <Card className="overflow-hidden border-l-0 border-r-0 border-t border-b-0 rounded-none shadow-none">
      ...
      <CardContent
        className={dbUserId === ownUser.id ? "pb-4 sm:pb-6" : "p-4 sm:p-6"}
      >
        <div className="space-y-4">
          <div>
            <div className="flex space-x-2">
              <Link href={`/profile/${ownUser.username}`}>
                <Avatar className="w-12 h-12 border-4 border-transparent">
                  <AvatarImage src={ownUser.image ?? "/avatar.png"} />
                </Avatar>
              </Link>
